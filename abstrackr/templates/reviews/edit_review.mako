...
<table class="form_table">
 ${h.form(url(controller='review', action='edit_review_settings', id=c.review.id), multipart=True, id="edit_project_form",  method='post')}
    ...
    <tr><td><label>screening mode (<a href="#" id="screen-mode-link">what?</a>):</td> <td>${h.select("screen_mode", c.screening_mode, ["Single-screen", "Double-screen", "Advanced"])} </label></td></tr>

    <tr><td><label>order abstracts by:</td> <td>${h.select("order", c.order_abstracts, ["Most likely to be relevant", "Random", "Most ambiguous"])} </label></td></tr>
    ...
    <tr><td><label>tag visibility (<a href="#" id="tag-visibility-link">what?</a>):</td> <td>${h.select("tag_visibility", c.tag_privacy, ["Private", "Public"])} </label></td></tr>
