...
class CollectiveAuthorLearner(SimpleLearner):
        ...
        def ICA_train(self, iters=50):
            # this is just using the BOW 
            if self.undersampled is None:
                self.undersampled = self.rebuild_models(for_eval=True)
            else:
                self.rebuild_models(for_eval=True, undersample_these=self.undersampled)
            
                
            gid = random.choice(self.gid_to_id.keys())
